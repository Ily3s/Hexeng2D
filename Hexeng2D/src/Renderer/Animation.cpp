...
namespace Hexeng::Renderer
{
	...
	Animation::Animation(std::vector<AnimationStep> steps_p, std::function<void(void)> terminate_p)
		: steps(steps_p), terminate(terminate_p) {}
	...
	void Animation::update_animations()
	{
		for (Animation* anim : Animation::animations)
		{
			...
			if (anim->time >= anim->steps[anim->current_step].timestamp)
			{
				...
				anim->steps[anim->current_step].up_anim(anim->time - anim->steps[anim->current_step].timestamp);
				continue;
			}
