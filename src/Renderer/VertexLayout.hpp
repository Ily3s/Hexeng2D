	...
	public:
		...
		void bind() const
		{
			...
			for (unsigned int i = 0; i < m_elements.size(); ++i)
			{
				...
				HXG_SGL(glEnableVertexAttribArray(i));
				HXG_SGL(glVertexAttribPointer(i, element.count, element.type, GL_FALSE, m_stride, (const void*)offset));
		...
		void unbind() const
		{
			for (unsigned int i = 0; i < m_elements.size(); ++i)
			{
				HXG_SGL(glDisableVertexAttribArray(i));
			}
		}
	};
