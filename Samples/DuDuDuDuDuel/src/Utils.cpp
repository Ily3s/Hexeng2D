...
DefaultButton::DefaultButton(const std::u32string& str, std::function<void(void)> on_click, Hexeng::Vec2<int> pos,
	...
	text(&language_switch, str, roboto_font, { pos.x, pos.y - (int)(2 * size) }, 7 * size),
...
Cursor::Cursor(const std::u32string& str, float* var_p, Hexeng::Vec2<int> pos, Hexeng::Vec2<int> size,
	Hexeng::Color4 color, Hexeng::Range range)
	: mesh({ &stick, &text, &quad }), var(var_p),
	text(&language_switch, str, roboto_font, { pos.x - static_cast<int>(4.0f / 10.0f * size.x),
		pos.y + static_cast<int>(5.0f / 8.0f * size.y) }, static_cast<int>(1.0f / 2.0f * size.y)),
	quad({ stick.get_min().x + (int)((*var) * stick.get_size().x), pos.y }, size.y, nullptr),
	stick(pos, { size.x, static_cast<int>(1.0f / 3.0f * size.y) }, nullptr),
	evt({ pos - Hexeng::Vec2<int>{size.x + size.y, size.y} / 2 }, { pos + Hexeng::Vec2<int>{size.x + size.y, size.y} / 2 },
		{
