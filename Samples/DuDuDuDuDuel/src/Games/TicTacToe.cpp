...
namespace TicTacToe
{
	...
	void check_for_wins()
	{
		for (int i = 0; i < 3; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				p1_win = p1_win && (game_mesh_grid[i][j].get_texture() == &circle) && (game_mesh_grid[i][j].enable);
				p2_win = p2_win && (game_mesh_grid[i][j].get_texture() == &cross) && (game_mesh_grid[i][j].enable);
			}
			if (p1_win)
			{
				player1_wins();
		...
		{
			if ((game_mesh_grid[0][0].get_texture() == &circle) && (game_mesh_grid[1][1].get_texture() == &circle) && (game_mesh_grid[2][2].get_texture() == &circle))
			{
				player1_wins();
				return;
			}

			if ((game_mesh_grid[0][0].get_texture() == &cross) && (game_mesh_grid[1][1].get_texture() == &cross) && (game_mesh_grid[2][2].get_texture() == &cross))
			{
				player2_wins();
				return;
			}
		...
		{
			if ((game_mesh_grid[0][2].get_texture() == &circle) && (game_mesh_grid[1][1].get_texture() == &circle) && (game_mesh_grid[2][0].get_texture() == &circle))
			{
				player1_wins();
				return;
			}

			if ((game_mesh_grid[0][2].get_texture() == &cross) && (game_mesh_grid[1][1].get_texture() == &cross) && (game_mesh_grid[2][0].get_texture() == &cross))
			{
				player2_wins();
				return;
			}
	...
	Hexeng::Renderer::ToBeInit init_tic_tac_toe
	{
		[]()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				for (int j = 0; j < 3; j++)
				{
					game_mesh_grid[i][j] = Hexeng::Renderer::Quad{ {-250 + i * 250, -250 + j * 250 }, 1.0f, &circle };
					game_mesh_grid[i][j].enable = false;
					...
					game_evt_grid[i][j] = Hexeng::EventManager::Button{
						...
						{{Hexeng::EventManager::ButtonEvent::CLICK, [i, j]()
						{
							...
							if (!(game_mesh_grid[i][j].enable || stop_game))
							{
								if (turn)
								{
									game_mesh_grid[i][j].access_texture() = &circle;
									whos_turn.access_texture() = &cross;
								}
								else
								{
									game_mesh_grid[i][j].access_texture() = &cross;
									whos_turn.access_texture() = &circle;
								}
